...
The Local connector implements the VolumeConnector interface for FusionStorage direct-attach (local SCSI) volumes.
...
#### Scenario: Attach local volume with device discovery details
- **WHEN** the connector attaches a local SCSI volume
- **THEN** it performs the following:
  - Polls /dev/disk/by-id/wwn-0x<tgtLunWWN> up to 30 times with 2-second intervals (60 seconds total) via waitDevOnline
  - Resolves the symlink to get the actual device path (e.g., /dev/sdX) for device verification
  - Returns empty string (not error) if timeout
