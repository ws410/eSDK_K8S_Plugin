...
The NFS connector implements the VolumeConnector interface to mount/unmount NFS filesystem shares on Kubernetes nodes. It supports both block device mounting (srcType=block) and filesystem mounting (srcType=fs).
